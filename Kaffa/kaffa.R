...
ggplot(yhteenveto) +
  ...
  geom_line(data=subset(yhteenveto2, MAA!="INFO"), aes(y=rahaa, x=kiloja, group = MAA)) + 
  #COUNTRY
  geom_text(data=subset(yhteenveto, kiloja<10000 | kiloja>20000), aes(y=rahaa+8000, x=kiloja, label=MAA), family="Unica One", size=6) +
  geom_text(data=subset(yhteenveto, MAA=="INDONESIA"), aes(y=rahaa,   x=kiloja-600, label=MAA), family="Unica One", size=6, hjust=1) +
  geom_text(data=subset(yhteenveto, MAA=="ETHIOPIA"),  aes(y=rahaa,   x=kiloja+600, label=MAA), family="Unica One", size=6, hjust=0) +
  geom_text(data=subset(yhteenveto, MAA=="UGANDA"),    aes(y=rahaa,   x=kiloja+600, label=MAA), family="Unica One", size=6, hjust=0) +
  geom_text(data=subset(yhteenveto, MAA=="INDIA"),     aes(y=FT-8000, x=kiloja,     label=MAA), family="Unica One", size=6) +
  #LEGEND
  annotate("text", x=1000, y=180000, label= "KAFFA HANDSHAKE (FOB)",     family="Unica One", size=6, hjust=0, col= "gray30") +
  annotate("text", x=1000, y=170000, label= "FAIR TRADE MINIMUM", family="Unica One", size=6, hjust=0, col= "gray30") +
  geom_point(data=yhteenveto2[20,], aes(y=rahaa, x=kiloja), size = 3, shape = 17,                   col= "gray30") +
  geom_point(data=yhteenveto2[20,], aes(y=FT,    x=kiloja), size = 3, shape = 16,                   col= "gray30") +
  geom_line(data=subset(yhteenveto2, MAA=="INFO"), aes(y=rahaa, x=kiloja, group = MAA),             col= "gray30") + 
  #SPECS
  scale_y_continuous(limits = c(0, 180000), breaks = c(0, 50000, 100000, 150000), labels = label_dollar()) +
  scale_x_continuous(limits = c(0, 40000),
                     breaks = c(0,       10000,       20000,       30000,       40000),
                     label = c("0 KG", "10 000 KG", "20 000 KG", "30 000 KG", "40 000 KG")) +
  labs(title    = "   KAFFA ROASTERY IN 2019: BOUGHT & PAID IN TOTAL BY COUNTRY", 
       subtitle = "    GREEN COFFEE PRICES: KAFFA HANDSHAKE vs FAIR TRADE",
       caption = "SOURCE: www.kaffaroastery.fi/raakakahvien-hinnat-2019") +
  xlab("") + ylab("") + theme_minimal() +
  theme(axis.title.y = element_text(angle = 0, vjust=0),
        axis.title.x = element_text(angle = 0, hjust=0),
        plot.subtitle = element_text(margin = unit(c(0, 0, 0, 0), "mm")),
        ...
        axis.ticks.x = element_line(),
        axis.ticks.y = element_line(),
        text=element_text(family="Unica One", size=22),
        plot.margin=unit(c(1, 1, .3, .3),"cm"), 
        plot.background = element_rect(fill = "darkkhaki", colour = NA),
        plot.caption = element_text(color = "gray40", family="Press Start 2P", size=7, hjust=0.5),
        legend.position = "none") 

ggsave(file="KAFFA2019a_twitter.jpg", width = (2/3)*16, height = 6, units = "in") 
